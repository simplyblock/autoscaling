...
RUN echo -e 'Acquire::Retries "5";' > /etc/apt/apt.conf.d/80-retries && \
...
RUN case "${PG_VERSION}" in \
    "v18") \
      ...
      tar xvfz postgres.tar.gz && \
      mv postgres-REL_18_1 postgres; \
      ;; \
    esac
RUN cd postgres && \
    ...
    *) \
    ...
    echo "No contrib patches for this PostgreSQL version";; \
    esac && \
...
RUN case "${PG_VERSION:?}" in \
    "v18") \
        export POSTGIS_VERSION=3.6.1 \
        export POSTGIS_CHECKSUM=ec0cfaab475630106211d180d71df46782e41e10b6dffe91d79ca818ecd2cbb4 \
    ;; \
    "v17") \
...
RUN case "${PG_VERSION:?}" in \
    "v18") \
        export PGROUTING_VERSION=4.0.0 \
        export PGROUTING_CHECKSUM=ae87d30652b4a7824509e2652e02bde19e1a42c37906cdf1824b5df40af0bfd0 \
    ;; \
    "v17") \
...
RUN wget https://github.com/pgvector/pgvector/archive/refs/tags/v0.8.1.tar.gz -O pgvector.tar.gz && \
    echo "a9094dfb85ccdde3cbb295f1086d4c71a20db1d26bf1d6c39f07a7d164033eb4 pgvector.tar.gz" | sha256sum --check && \
...
RUN wget https://github.com/postgrespro/rum/archive/refs/tags/1.3.15.tar.gz -O rum.tar.gz && \
    echo "e79b3a67df9821bc0d86fd463dac7249f1729d9dd04f77db767e2815098247b8 rum.tar.gz" | sha256sum --check && \
...
WORKDIR /ext-src
RUN wget https://github.com/okbob/plpgsql_check/archive/refs/tags/v2.8.5.tar.gz -O plpgsql_check.tar.gz && \
    echo "0328b8b035ca0474ec451a15d1a1b05d4796cee6adfcbdb6af85bf77e03d16b9 plpgsql_check.tar.gz" | sha256sum --check && \
...
RUN case "${PG_VERSION:?}" in \
      ...
      "v17") \
        ...
        ;; \
      "v18") \
        export TIMESCALEDB_VERSION=2.24.0 \
        export TIMESCALEDB_CHECKSUM=06a5d38c52bebb364104541002ccfb9ea739f93183c339895f0d1ed90b005e17 \
        ;; \
    esac && \
...
WORKDIR /ext-src
COPY ./patches/pg_cron.patch .
RUN wget https://github.com/citusdata/pg_cron/archive/refs/tags/v1.6.7.tar.gz -O pg_cron.tar.gz && \
    echo "d950bc29155f31017567e23a31d268ff672e98276c0e9d062512fb7870351f03 pg_cron.tar.gz" | sha256sum --check && \
...
WORKDIR /ext-src
COPY ./patches/anon_v2.patch .

# This is an experimental extension, never got to real production.
# !Do not remove! It can be present in shared_preload_libraries and compute will fail to start if library is not found.
ENV PATH="/usr/local/pgsql/bin/:$PATH"
RUN wget https://gitlab.com/dalibo/postgresql_anonymizer/-/archive/2.5.1/postgresql_anonymizer-latest.tar.gz -O pg_anon.tar.gz && \
    echo "93643b486b710baf325d1d05707b9eaa25ef328a622eec1f04f6f4a7d308aaea  pg_anon.tar.gz" | sha256sum --check && \
...
WORKDIR /ext-src
RUN wget https://github.com/sraoss/pg_ivm/archive/refs/tags/v1.13.tar.gz -O pg_ivm.tar.gz && \
    echo "0868e5f6473d5de70e5314decd221ce4e7d2e1326ee99384fd593a0866bca1ab pg_ivm.tar.gz" | sha256sum --check && \
...
WORKDIR /ext-src
COPY ./patches/pg_duckdb_v031.patch .
COPY ./patches/duckdb_v120.patch .
...
RUN case "${PG_VERSION}" in \
    "v14" | "v15" | "v16" | "v17" | "v18") \
    export PGAUDITLOGTOFILE_VERSION=v1.7.6 \
    export PGAUDITLOGTOFILE_CHECKSUM=a916365dcd6e219ddf4db7405a16323da8e7d44ca0d8f20d2349aaf3982e43b1 \
    ;; \
    *) \
...
RUN if [ "$TARGET_ARCH" = "amd64" ]; then\
    ...
    && curl -sL https://github.com/prometheus-community/postgres_exporter/releases/download/v0.17.1/postgres_exporter-0.17.1.linux-${TARGET_ARCH}.tar.gz \
     | tar xzf - --strip-components=1 -C.\
    && curl -sL https://github.com/prometheus-community/pgbouncer_exporter/releases/download/v0.10.2/pgbouncer_exporter-0.10.2.linux-${TARGET_ARCH}.tar.gz \
     | tar xzf - --strip-components=1 -C.\
    && curl -sL https://github.com/burningalchemist/sql_exporter/releases/download/0.17.3/sql_exporter-0.17.3.linux-${TARGET_ARCH}.tar.gz \
     | tar xzf - --strip-components=1 -C.\
...
#COPY --from=timescaledb-src /ext-src/ /ext-src/
COPY --from=pg_hint_plan-src /ext-src/ /ext-src/
COPY ./patches/pg_hint_plan_${PG_VERSION:?}.patch /ext-src
...
RUN curl -L -o vector.deb "https://packages.timber.io/vector/0.52.0/vector_0.52.0-1_${TARGET_ARCH}.deb" && \
    dpkg -i vector.deb && \
    rm vector.deb
...
COPY --chown=postgres ./etc/postgres_exporter.yml /etc/postgres_exporter.yml
